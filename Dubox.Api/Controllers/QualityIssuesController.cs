    ...
    public class QualityIssuesController : ControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (Request.HasFormContentType)
            {
                ...
                // NCR is optional - default Internal (1)
                var ncrValue = form["NCR"].FirstOrDefault();
                if (!string.IsNullOrWhiteSpace(ncrValue) && Enum.TryParse<NCRTypeEnum>(ncrValue, out var ncrParsed))
                    ncr = ncrParsed;
