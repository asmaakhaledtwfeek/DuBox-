...
public class ScanPanelBarcodeCommandHandler : IRequestHandler<ScanPanelBarcodeCommand, Result<BoxPanelDto>>
{
    ...
    public async Task<Result<BoxPanelDto>> Handle(ScanPanelBarcodeCommand request, CancellationToken cancellationToken)
    {
        ...
        // First approval (via scan) only when panel is completed: Completed or FirstApprovalPending
        bool canAutoApproveFirst = (panel.PanelStatus == PanelStatusEnum.Completed || panel.PanelStatus == PanelStatusEnum.FirstApprovalPending)
            && (panel.FirstApprovalStatus != "Approved" && panel.FirstApprovalStatus != "Rejected");

        if (canAutoApproveFirst)
        {
            // Automatically approve first approval when panel is completed and first approval not yet done
            panel.FirstApprovalStatus = "Approved";
            panel.FirstApprovalBy = currentUserId;
            panel.FirstApprovalDate = scanTime;
            panel.FirstApprovalNotes = $"Automatically approved on scan at {scanTime:yyyy-MM-dd HH:mm:ss} UTC";
