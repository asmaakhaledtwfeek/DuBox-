...
public class CreateProgressUpdateCommandHandler : IRequestHandler<CreateProgressUpdateCommand, Result<ProgressUpdateDto>>
{
    ...
    private async Task<Result> ValidateProgressUpdateAsync(CreateProgressUpdateCommand request, BoxActivity boxActivity, CancellationToken cancellationToken)
    {
        ...
        if (boxPanels.Any())
        {
            var panelsNotSecondApproved = boxPanels.Where(p => p.PanelStatus != PanelStatusEnum.SecondApprovalApproved).ToList();
            if (panelsNotSecondApproved.Any())
                return Result.Failure("Cannot update activity progress. All panels in this box must have Second Approval Approved before updating progress. One or more panels are not yet approved at Second Approval.");
        }
        ...
        // When updating progress to 100%, require at least one image in this activity's progress updates (current request or previous)
        if (request.ProgressPercentage >= 100)
        {
            ...
            if (!hasImageInCurrentRequest)
            {
                ...
                bool hasExistingImage = existingUpdateIds.Count > 0 && await _dbContext.ProgressUpdateImages
                ...
                bool hasExistingPhoto = await _dbContext.ProgressUpdates
                    .AnyAsync(pu => pu.BoxActivityId == request.BoxActivityId && !string.IsNullOrWhiteSpace(pu.Photo), cancellationToken);

                if (!hasExistingImage && !hasExistingPhoto)
                    return Result.Failure("You must add one or more images to a progress update before completing this activity (100%). Add at least one progress photo and try again.");
            }
        }
