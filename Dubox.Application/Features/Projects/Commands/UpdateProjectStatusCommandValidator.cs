    ...
    public class UpdateProjectStatusCommandValidator : AbstractValidator<UpdateProjectStatusCommand>
    {
        ...
        private async Task<bool> AllowCompletedStatusWhenConditionsMet(UpdateProjectStatusCommand command, CancellationToken cancellationToken)
        {
            ...
            if (project.Status == ProjectStatusEnum.Closed && project.ProgressPercentage >= 100)
            {
                ...
                {
                    if (!project.AllowCompletionWithConditionalApproval)
                    {
                        var boxIds = boxes.Select(b => b.BoxId).ToList();
                        var conditionalApprovalCheckpoints = await _unitOfWork.Repository<WIRCheckpoint>()
                            .FindAsync(w => boxIds.Contains(w.BoxId) && w.Status == WIRCheckpointStatusEnum.ConditionalApproval, cancellationToken);
                        if (conditionalApprovalCheckpoints.Any())
                            return false;
                    }
