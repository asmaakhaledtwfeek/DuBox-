        ...
        try
        {
            ...
            foreach (var wirMaster in wirMasters)
            {
                ...
                var categories = await _unitOfWork.Repository<Category>()
                    .FindAsync(c => categoryIds.Contains(c.CategoryId), cancellationToken);
                var references = await _unitOfWork.Repository<Reference>()
                    .FindAsync(r => referenceIds.Contains(r.ReferenceId), cancellationToken);

                var categoryDict = categories.ToDictionary(c => c.CategoryId, c => c);
                var referenceDict = references.ToDictionary(r => r.ReferenceId, r => r);
