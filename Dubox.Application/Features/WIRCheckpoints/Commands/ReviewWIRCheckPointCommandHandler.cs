    ...
    : IRequestHandler<ReviewWIRCheckPointCommand, Result<WIRCheckpointDto>>
    {
        ...
        public async Task<Result<WIRCheckpointDto>> Handle(ReviewWIRCheckPointCommand request, CancellationToken cancellationToken)
        {
            ...
            if (currentUserId == Guid.Empty)
                return Result.Failure<WIRCheckpointDto>("Access denied. User not identified.");

            // Allow: assigned inspector, system admin, project creator (created by), or project manager
            var isAssignedInspector = wir.InspectorId.HasValue && wir.InspectorId.Value == currentUserId;
            var isSystemAdmin = await _userRoleService.UserHasRoleAsync(currentUserId, nameof(SystemRoleEnum.SystemAdmin), cancellationToken);
            var project = wir.Box?.Project;
            var isProjectCreator = project?.CreatedBy.HasValue == true && project.CreatedBy!.Value == currentUserId;
            var isProjectManager = project?.ProjectMangerId.HasValue == true && project.ProjectMangerId!.Value == currentUserId;

            if (!isAssignedInspector && !isSystemAdmin && !isProjectCreator && !isProjectManager)
                return Result.Failure<WIRCheckpointDto>("Access denied. Only the assigned inspector, system admin, or project manager (created by) can review this checkpoint.");
