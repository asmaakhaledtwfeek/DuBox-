...
public class ProjectProgressService : IProjectProgressService
{
    ...
    public async Task<(decimal? oldProgress, decimal newProgress)> UpdateProjectProgressAsync(Guid projectId, Guid changedBy, string reason, CancellationToken cancellationToken)
    {
        ...
        {
            var canCompleteWithConditionalApproval = project.AllowCompletionWithConditionalApproval;
            if (canCompleteWithConditionalApproval)
            {
            ...
            else
            {
                ...
                var hasConditionalApproval = (await _unitOfWork.Repository<WIRCheckpoint>()
                    .FindAsync(w => boxIds.Contains(w.BoxId) && w.Status == WIRCheckpointStatusEnum.ConditionalApproval, cancellationToken)).Any();
