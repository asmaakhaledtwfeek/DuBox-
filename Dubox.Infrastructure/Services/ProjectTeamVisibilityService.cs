...
public class ProjectTeamVisibilityService : IProjectTeamVisibilityService
{
    ...
    public async Task<bool> CanEditProjectAsync(Guid projectId, CancellationToken cancellationToken = default)
    {
        if (!_currentUserService.IsAuthenticated || string.IsNullOrEmpty(_currentUserService.UserId))
        {
            return false;
        ...
        // SystemAdmin can edit all projects
        var isSystemAdmin = await _userRoleService.UserHasRoleAsync(userId, SystemAdminRole, cancellationToken);
        if (isSystemAdmin)
        {
            return true;
        ...
        // Get all user roles to check if user has ONLY Viewer role
        var userRoles = (await _userRoleService.GetUserRolesAsync(userId, cancellationToken)).ToList();
        var hasOnlyViewerRole = userRoles.Count == 1 && userRoles.Contains(ViewerRole);

        // If user has ONLY Viewer role (no other roles), they cannot edit any project
        if (hasOnlyViewerRole)
        {
            return false;
        }

        // Load the project to check who created it
        ...
        // User can edit projects they created
        if (project.CreatedBy == userId.ToString())
        {
            return true;
        }

        // Check if project was created by a ProjectManager or SystemAdmin who created a team the user is a member of
        if (!Guid.TryParse(project.CreatedBy, out var projectCreatorId))
        {
            return false;
        }

        // Check if the project creator is a ProjectManager or SystemAdmin
        var creatorIsProjectManager = await _userRoleService.UserHasRoleAsync(projectCreatorId, ProjectManagerRole, cancellationToken);
        var creatorIsSystemAdmin = await _userRoleService.UserHasRoleAsync(projectCreatorId, SystemAdminRole, cancellationToken);

        if (!creatorIsProjectManager && !creatorIsSystemAdmin)
        {
            // Project creator is not a PM or SA, so user cannot edit
            return false;
        }

        // Check if user is a member of any team created by the project creator
        var userTeams = await _context.TeamMembers
            .Where(tm => tm.UserId == userId && tm.IsActive)
            .Select(tm => tm.Team)
            .ToListAsync(cancellationToken);

        var isMemberOfCreatorTeam = userTeams.Any(t => t.CreatedBy == projectCreatorId);

        return isMemberOfCreatorTeam;
    }
