...
import { BoxActivity, Box, BoxStatus, PanelStatus, canPerformActivityActions, getAvailableActivityStatuses, canPerformActivityActionsByStatus, ActivityStatus } from '../../../core/models/box.model';
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, ReactiveFormsModule, FormsModule, HeaderComponent, SidebarComponent, UpdateProgressModalComponent, ProgressUpdatesTableComponent, DateTimeDisplayPipe],
...
export class ActivityDetailsComponent implements OnInit, OnDestroy {
  ...
  private updatePermissions(): void {
    ...
    // Hide it if activity is completed/delayed AND next WIR has position, or if not all panels are completed
    const shouldHideUpdateProgress = this.shouldHideUpdateProgressButton();
    const allPanelsCompleted = this.areAllPanelsCompleted();
    this.canUpdateProgress = baseCanPerformActions && hasUpdateProgressPermission && !shouldHideUpdateProgress && allPanelsCompleted;
  ...
  formatDate(date?: Date): string {
    return formatDateWithTime(date, 'Not set');
  }
  ...
  /**
   * Whether all panels in the box have Second Approval Approved (required before updating activity progress).
   */
  areAllPanelsCompleted(): boolean {
    if (!this.box?.boxPanels || this.box.boxPanels.length === 0) {
      return true;
    }
    return this.box.boxPanels.every(panel =>
      panel.panelStatus === PanelStatus.SecondApprovalApproved ||
      (panel.panelStatus as number) === 9 ||
