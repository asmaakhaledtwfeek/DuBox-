...
export class ActivityTableComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Input() allPanelsCompleted: boolean = true; // Check if all panels have status Completed (required for progress update)
  @Input() incompletePanelsCount: number = 0; // Count of panels not completed (for tooltip)
  ...
  /**
   * Format date with time (date on first line, hours:minutes on second line)
   */
  ...
  getUpdateProgressButtonTitle(activity: BoxActivityDetail): string {
    ...
    // Check if all panels have Second Approval Approved
    if (!this.allPanelsSecondApproved) {
      const count = this.panelsNeedingApproval;
      return `Update Progress disabled: ${count} panel${count !== 1 ? 's' : ''} require${count === 1 ? 's' : ''} second approval before progress can be updated.`;
    }
