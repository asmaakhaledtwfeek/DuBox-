...
import { AuditLogDetailsModalComponent } from '../../../admin/audit-logs/audit-log-details-modal/audit-log-details-modal.component';
import type { UserDto } from '../../../../core/services/user.service';
...
@Component({
  ...
  styleUrls: ['./user-audit-logs.component.scss']
})
export class UserAuditLogsComponent implements OnInit {
  userId!: string;
  ...
  loadAuditLogs(): void {
    ...
    this.auditLogService.getAuditLogs(params).subscribe({
      next: (response) => {
        ...
        // Update available filters from current page
  ...
  async exportToExcel(): Promise<void> {
    ...
    const userName = this.user?.fullName || this.user?.email || 'User';
    const fileName = `Audit_Logs_${userName.replace(/[^a-zA-Z0-9]/g, '_')}_${dateStr}.xlsx`;
