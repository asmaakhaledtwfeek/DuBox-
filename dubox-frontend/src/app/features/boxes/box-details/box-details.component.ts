...
import { QualityIssueDetails, QualityIssueStatus, UpdateQualityIssueStatusRequest, WIRCheckpoint, WIRCheckpointStatus, WIRRecord, CreateQualityIssueForBoxRequest, IssueType, SeverityType, NCRType } from '../../../core/models/wir.model';
...
export class BoxDetailsComponent implements OnInit, OnDestroy {
  ...
  // Full factory layout in Box Information (only current box shown in grid)
  ...
  loadBox(): void {
    ...
    this.boxService.getBox(this.boxId).subscribe({
      next: (box) => {
        ...
        // Load factory for small layout when box has factory and position
        if (box.factoryId && (box.bay || box.row)) {
          this.loadFactoryForLayout();
        } else {
          this.factory = null;
  ...
  /**
   * Load factory for the full "Location in Factory" layout (min/max bay and row).
   */
  private loadFactoryForLayout(): void {
  ...
  /**
   * Build the full factory layout (all bays and rows from factory config) with only the current box in its cell.
   */
  getBoxLocationGridLayout(): { columns: string[]; rows: string[]; matrix: { row: string; column: string; bay: string; box: Box | null; isCurrentBox: boolean }[][] } {
    if (!this.box) {
      return { columns: [], rows: [], matrix: [] };
    }
    const minRow = this.factory?.minRow ?? 1;
    const maxRow = this.factory?.maxRow ?? 20;
    const minBay = (this.factory?.minBay ?? 'A').toString().toUpperCase();
    const maxBay = (this.factory?.maxBay ?? 'Z').toString().toUpperCase();
    ...
    const currentBay = (this.box.bay || '').toString().toUpperCase();
    const currentRow = (this.box.row || '').toString();

    const matrix: { row: string; column: string; bay: string; box: Box | null; isCurrentBox: boolean }[][] = [];
    rows.forEach(r => {
      const rowCells: { row: string; column: string; bay: string; box: Box | null; isCurrentBox: boolean }[] = [];
      columns.forEach(c => {
        const isCurrentBox = c === currentBay && r === currentRow;
        const boxToShow = isCurrentBox ? this.box! : null;
        rowCells.push({
          row: r,
          column: c,
          bay: c,
  ...
  createQualityIssue(): void {
    ...
    const request: CreateQualityIssueForBoxRequest = {
      ...
      ncr: this.newQualityIssueForm.ncr,
...
downloadFileDrawing(drawing: any): void {
  ...
  submitStatusUpdate(): void {
    ...
    this.wirService.updateQualityIssueStatus(
      ...
      error: (err) => {
        ...
        this.statusUpdateLoading = false;
        this.statusUpdateError = extractErrorMessage(err, 'Failed to update issue status');
        // Error toast shown by ApiService.handleError
      }
    });
  }
  ...
  /**
   * Whether all panels in the box have status Completed (required before updating activity progress).
   */
  areAllPanelsCompleted(): boolean {
    if (!this.boxPanels || this.boxPanels.length === 0) {
      return true;
    }
    return this.boxPanels.every(panel =>
      panel.panelStatus === PanelStatus.Completed || (panel.panelStatus as number) === 3
    );
  }

  /**
   * Count of panels that are not completed (used for tooltip).
   */
  getIncompletePanelsCount(): number {
    if (!this.boxPanels || this.boxPanels.length === 0) {
      return 0;
    }
    return this.boxPanels.filter(panel =>
