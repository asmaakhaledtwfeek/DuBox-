...
export class BoxDetailsComponent implements OnInit, OnDestroy {
  ...
  exportWIRToPDF(checkpoint: WIRCheckpoint): void {
    // Create a printable view of the WIR checkpoint
    ...
    const content = this.generateWIRPrintContent(checkpoint);
...
<body>
  ...
  private generateWIRPrintContent(checkpoint: WIRCheckpoint): string {
    ...
    let html = `
      <div class="inspection-form-container">
        ...
        <div class="project-info-section">
          <table class="info-table">
            ...
            <tr>
              ...
              <td class="value">${this.formatWIRDate(checkpoint.requestedDate) || '_____'}</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td class="value">${this.box?.code || this.box?.name || '_____'}</td>
            </tr>
            <tr>
        ...
        <div class="activity-section">
          <strong>Activity/Item:</strong> ${checkpoint.wirDescription || checkpoint.wirName}
        </div>

        <!-- Checklist Items Table with Categories -->
    ...
    for (const [category, categoryItems] of Object.entries(groupedItems)) {
      ...
      categoryItems.forEach((item) => {
        ...
        html += `
              <tr>
                <td class="col-sr">${itemCounter}</td>
                <td class="col-desc">${item.checkpointDescription}</td>
                <td class="col-check">
                  <div class="checkbox-cell">
                    <div class="check-mark">${isPassed ? '<span>✓</span>' : ''}</div>
                  </div>
                </td>
                <td class="col-check">
                  <div class="checkbox-cell">
                    <div class="check-mark">${isFailed ? '<span>✗</span>' : ''}</div>
                  </div>
                </td>
                <td class="col-check">
                  <div class="checkbox-cell">
                    <div class="check-mark">${isPending ? '<span>-</span>' : ''}</div>
                  </div>
                </td>
    ...
    html += `
        ...
        <div class="comments-section">
          ...
          <div class="comments-box">
            <p>${checkpoint.comments || 'No comments'}</p>
          </div>
        </div>
        ...
        <div class="signature-section">
          ...
          <!-- First Row: Finish Date and QA/QC Engineer -->
          <div class="signature-grid">
            <div class="signature-block">
              <div class="block-title">Finish Date:</div>
              <div class="field-row">
                <label>Name:</label>
              ...
              <div class="field-row">
                ...
                <div class="signature-input">0</div>
              </div>
              <div class="field-row">
            ...
            <div class="signature-block">
              <div class="block-title">QA/QC Engineer</div>
              <div class="field-row">
                <label>Name:</label>
                <div class="signature-input">${checkpoint.inspectorName || ''}</div>
              </div>
              <div class="field-row">
                <label>Position:</label>
                <div class="signature-input">QA/QC Engineer</div>
              </div>
              <div class="field-row">
                <label>Signature:</label>
                <div class="signature-box"></div>
              ...
              <div class="field-row">
                <label>Date:</label>
                <div class="signature-input">${this.formatWIRDate(checkpoint.inspectionDate) || ''}</div>
              </div>
            </div>
          </div>

          <!-- Second Row: Approval Team -->
          <div class="signature-grid">
            <div class="signature-block optional">
              <div class="block-title">Approval Team:</div>
              <div class="field-row">
                <label>Name:</label>
                <div class="signature-input"></div>
              </div>
  ...
  private groupChecklistItemsByCategory(items: any[]): Record<string, any[]> {
    ...
    // Sort items within each category by sequence
    Object.keys(grouped).forEach(category => {
      grouped[category].sort((a, b) => (a.sequence || 0) - (b.sequence || 0));
    });
  ...
  private getProjectInfo() {
    return {
      ...
      location: 'DUBOX Reign'
    };
  }

  private extractReferenceDocuments(items: any[]): string {
    const refs = new Set<string>();
    items.forEach(item => {
      if (item.referenceDocument && item.referenceDocument !== '-' && item.referenceDocument !== 'General') {
  ...
  private getWIRPrintStyles(): string {
    return `
      * { margin: 0; padding: 0; box-sizing: border-box; }
      ...
      .logo-box.logo-amaala .logo-text {
        font-size: 20px;
        font-weight: 900;
        color: #1a1a1a;
        letter-spacing: 3px;
      ...
      .checklist-sections-form .section-block {
      ...
      .checklist-items-table th.col-sr { 
        width: 35px;
        text-align: center;
      }
      
      .checklist-items-table th.col-desc { 
        width: auto;
        min-width: 350px;
        text-align: left;
      }
      
      .checklist-items-table th.col-ref { 
        width: 80px;
        display: none;
      }
      
      .checklist-items-table th.col-check { 
        width: 30px;
        text-align: center;
        font-weight: bold;
      }
      
      .checklist-items-table th.col-remarks { 
        width: 120px;
        text-align: center;
      }
      ...
      .checklist-items-table tbody tr:nth-child(even) {
        background: white;
      }
      
      .checklist-items-table tbody tr.category-header-row {
        background: linear-gradient(to bottom, #e8e8e8 0%, #d0d0d0 100%);
        border-top: 2px solid #000;
      }
      
      .checklist-items-table tbody td.category-header-cell {
        padding: 10px 15px;
        font-weight: bold;
        font-size: 12px;
        text-align: left;
      ...
      .checklist-items-table tbody td.category-header-cell strong {
        font-weight: bold;
        color: #000;
      }
      
      .checklist-items-table td.col-sr {
        text-align: center;
        font-weight: normal;
        color: #000;
        font-size: 10px;
      ...
      .checklist-items-table td.col-desc {
        line-height: 1.4;
        font-size: 11px;
        text-align: left;
      }
      
      .checklist-items-table td.col-ref {
        font-size: 10px;
        color: #000;
        text-align: center;
        display: none;
      ...
      .checklist-items-table td.col-check {
        text-align: center;
        padding: 3px;
        vertical-align: middle;
      }
      ...
      .signature-section .section-title-bar {
        display: block;
        background: linear-gradient(to bottom, #f5f5f5 0%, #e8e8e8 100%);
        color: #000;
        padding: 10px 15px;
      ...
      .signature-section .section-title-bar h2 {
        margin: 0;
        font-size: 12px;
        font-weight: bold;
        text-transform: uppercase;
