...
export class QaQcChecklistComponent implements OnInit, OnDestroy {
  ...
  readonly ncrTypes: NCRType[] = ['Internal', 'External'];
...
console.log(expectedWirCode);
  ...
  get isCurrentUserAssignedInspector(): boolean {
    // System Admins and Project Managers can always review/edit checkpoints
    if (this.authService.isSystemAdmin() || this.authService.isProjectManager()) {
      return true;
    }
    ...
    if (inspectorId && currentUserId.toLowerCase() === inspectorId.toLowerCase()) return true;
    // Project creator (created by)
    const projectCreatedBy: string | undefined = this.wirCheckpoint?.projectCreatedBy?.toString?.();
    if (projectCreatedBy && currentUserId.toLowerCase() === projectCreatedBy.toLowerCase()) return true;
    // Project manager on this project
    const projectManagerId: string | undefined = this.wirCheckpoint?.projectManagerId?.toString?.();
    if (projectManagerId && currentUserId.toLowerCase() === projectManagerId.toLowerCase()) return true;
    return false;
  }
   ...
   */
  isProjectManagerRole(): boolean {
    return this.authService.isProjectManager();
  }

  /**
   * Check if current user is project creator or project manager for this checkpoint's project (by project createdBy / projectManagerId)
   */
  isProjectCreatorOrManagerForCheckpoint(): boolean {
    const currentUser: any = this.authService.getCurrentUser();
    const currentUserId: string | undefined = (currentUser?.userId || currentUser?.id)?.toString?.();
    if (!currentUserId) return false;
    const projectCreatedBy = this.wirCheckpoint?.projectCreatedBy?.toString?.();
    const projectManagerId = this.wirCheckpoint?.projectManagerId?.toString?.();
    return (
      (!!projectCreatedBy && currentUserId.toLowerCase() === projectCreatedBy.toLowerCase()) ||
      (!!projectManagerId && currentUserId.toLowerCase() === projectManagerId.toLowerCase())
    );
  }
