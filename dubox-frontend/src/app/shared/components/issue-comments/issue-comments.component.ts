...
export class IssueCommentsComponent implements OnInit, OnChanges {
  ...
  addComment(): void {
    ...
    const validationResult = MeaningfulTextValidator.isValid(this.newCommentText);
    if (!validationResult.isValid) {
      this.toastService.showError(validationResult.errorMessage || 'Invalid comment text');
      return;
    }
  ...
  saveEdit(): void {
    ...
    // Validate meaningful text
    const validationResult = MeaningfulTextValidator.isValid(this.editedCommentText);
    if (!validationResult.isValid) {
      this.toastService.showError(validationResult.errorMessage || 'Invalid comment text');
      return;
    }
