...
export class QualityIssueDetailsModalComponent implements OnInit, OnChanges {
  ...
  async updateStatus(): Promise<void> {
    ...
    // Validate resolution description if required
    if (this.requiresResolution()) {
      const resolutionValidation = MeaningfulTextValidator.isValid(this.resolutionDescription);
      if (!resolutionValidation.isValid) {
        this.toastService.showError(resolutionValidation.errorMessage || 'Invalid resolution description');
        return;
      }
    }

    // Validate status comment if provided
    if (this.statusComment.trim()) {
      const commentValidation = MeaningfulTextValidator.isValid(this.statusComment);
      if (!commentValidation.isValid) {
        this.toastService.showError(commentValidation.errorMessage || 'Invalid status comment');
        return;
      }
    }
    ...
    try {
      ...
      this.toastService.showError(extractErrorMessage(error, 'Failed to update status'));
    } finally {
